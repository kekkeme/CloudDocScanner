...
protocol SnapDelegate: class {
    func snappedImage(results: ImageScannerResults)
}
...
class EditScanViewController: UIViewController {
    ...
    lazy private var nextButton: UIBarButtonItem = {
        ...
        let button = UIBarButtonItem(title: title, style: .plain, target: self, action: #selector(snapView))
    ...
    @objc func snapView() {
        ...
        self.snapDelegate?.snappedImage(results: results)
    }
